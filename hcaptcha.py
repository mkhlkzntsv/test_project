...
def get_label(str):
    if 'click' in str:
        ...
        for key in dict.ru.keys():
            if key in str.lower():
                return dict.ru[key]
    return False

def getMethod(tp, thr, task):
    if tp == 'elephant':
        return getElephant(thr)
    if tp == 'seaplane':
        return getSeaplane(thr)
    else:
        return getPoint(thr, task)

def getPoint(thr, task_text):
    test = YOLO(None)
    abc = []
    for i in range(9):
        lab = get_label(task_text)
        if not lab:
            raise('No keys')
        with open(f'./model/{thr}-{i}.jpg', "rb") as file:
            data = file.read()
        if test.solution(data, label=lab):
            abc.append(str(i))
    return ','.join(abc)

def getElephant(thr):
    print('Elephant mode')
    el = de_stylized.ElephantsDrawnWithLeaves(None)
    lst = []
    for i in range(9):
...
def getSeaplane(thr):
    print('Seaplane Mode')
    el = kernel.RainbowSeaplane('./model/rainbow.yaml')
    lst = []
    for i in range(9):
        with open(f'./model/{thr}-{i}.jpg', "rb") as file:
            data = file.read()
